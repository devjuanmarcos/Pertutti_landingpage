...
interface ImageViewerProps {
  ...
  col?: boolean; // Optional prop to control column layout
}
...
const ImageViewer_Basic = ({
  ...
  showControls = true,
  thumbnailUrl,
  col = false, // Default to false for single column layout
}: ImageViewerProps) => {
      ...
      <DialogTrigger asChild>
        <div className={cn("cursor-pointer", className)}>
          ...
          <img
            ...
            width="100%"
            className={cn(
              `h-auto w-full rounded-lg  transition-opacity hover:opacity-90 ${col ? "max-h-[320px] cursor-pointer object-cover" : "object-contain"}`,
              classNameThumbnailViewer
            )}
